...
for(let i = 0 ; i < this.subysubsubtipos2.length ; i++ ){
    if( this.subysubsubtipos2[i].crmit_subname == "REFERIDO PROMOTOR" && this.subysubsubtipos2[i].crmit_subsubname == "CREA REFERIDO PROMOTOR"){
    ...
    }else if( this.subysubsubtipos2[i].crmit_subname == "REFERIDO PROMOTOR" && this.subysubsubtipos2[i].crmit_subsubname == "ACTUALIZA REFERIDO PROMOTOR"){
      tipi_actualizacion = this.subysubsubtipos2[i].crmit_codigounico;
      console.log("tipi_actualizacion: "+tipi_actualizacion);
      this.etapaprocesoventaid = this.subysubsubtipos2[i].etapaprocesoventaid;
    }
}
//valor_tipificacion = 'REFERIDO PROMOTOR_'+tipi_creacion+'_'+tipi_actualizacion;
valor_tipificacion = 'REFERIDO PROMOTOR';
  ...
  if(this.Segunda_validacion_Email(this.form.value.CorreoElectronico) == false ){
   ...
   if( this.campos_con_error.length != 0 ){ //Verifica si hay errores o errores de campos
    ...
    if (this.conEmail) {
      ...
      this.sendServ.send_Todos_Referidos_QA(sendd)// sendData4
        .subscribe(
          (res: any) => {
            console.log(res.status);
            if (res.status == 200) {
              this.Abreventana(this.form.value.Nombre, this.form.value.ApellidoPaterno, this.form.value.Telefono, this.form.value.CorreoElectronico, "", this.form.value.Telefono);
              this.sendServ.sendHubSpot(this.form.value.Nombre, this.form.value.ApellidoPaterno, this.form.value.Telefono, this.form.value.CorreoElectronico, "", this.form.value.Telefono)// this.form.value)
  ...
  //Cambiado -
   ...
   private showDialog(message: string){
       ...
       dialogRef.afterClosed().subscribe(result => {
         window.location.href = "/referido-promotor";
       });
      }

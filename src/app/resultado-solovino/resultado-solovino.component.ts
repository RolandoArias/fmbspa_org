...
@Component({
  selector: 'app-resultado-solovino',
  templateUrl: './resultado-solovino.component.html',
  styleUrls: ['./resultado-solovino.component.scss']
})
export class ResultadoSolovinoComponent implements OnInit {
  ...
  ngOnInit() {
      ...
      let h = JSON.parse(localStorage.getItem('search_registersolo_value'));
  ...
  showLead(values: any) {
    ...
    localStorage.setItem('lead_id_registersolo', values.source.value);
  }

  seleccionarLead(){
    let useLead = localStorage.getItem('lead_id_registersolo');
    let go = this;
